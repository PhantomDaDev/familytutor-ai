...
def seed_demo_data():
    ...
    subjects = {
        ...
        child2: [("Mathematics", "Volume"), ("History", "Primary Sources"), ("Geography", "Map Skills")],
    }

    sample_questions = {
        child1: [
            ("Mia has 24 stickers and gives away 8. How many are left?", "16", "16", True),
            ("Which fraction means one out of four equal parts?", "1/4", "1/2", False),
            ("Which animal group has feathers?", "Birds", "Birds", True),
            ("What happens to many puddles on a sunny day?", "They evaporate", "They evaporate", True),
            ("Which word describes the kite in 'The red kite flew high'?", "red", "red", True),
            ("Which sentence is capitalized correctly?", "My dog is named Spot.", "my dog is named Spot.", False),
        ],
        child2: [
            ("A box is 5 cm long, 3 cm wide, and 2 cm tall. What is its volume?", "30 cubic cm", "30 cubic cm", True),
            ("Which number is equal to 4.7?", "47 tenths", "47 hundredths", False),
            ("Why are primary sources useful when learning history?", "They come from the time being studied", "They come from the time being studied", True),
            ("Which action is an example of civic responsibility?", "Voting in an election", "Ignoring community rules", False),
            ("Which map feature explains what symbols on a map mean?", "Map key", "Map key", True),
            ("Which landform is a large, flat area higher than nearby land?", "Plateau", "Valley", False),
        ],
    }
    ...
    for child_id, subject_topic_pairs in subjects.items():
        for _ in range(12):
            ...
            for _ in range(total):
                question, correct_answer, user_answer, is_correct = random.choice(sample_questions[child_id])
...
def normalize_demo_children():
    from core.database import get_conn, get_default_topics
    ...
    kid2 = conn.execute("SELECT id FROM children WHERE name=?", ("Kid 2",)).fetchone()
    if kid2:
        child_id = kid2["id"]
        subjects = conn.execute("SELECT name FROM subjects WHERE child_id=?", (child_id,)).fetchall()
        desired_topics = [
            (subject["name"], topic)
            for subject in subjects
            for topic in get_default_topics(subject["name"], 5)
        ]
        current_topics = conn.execute(
            "SELECT subject, topic FROM leitner_topics WHERE child_id=?",
            (child_id,),
        ).fetchall()
        current_topic_set = {(row["subject"], row["topic"]) for row in current_topics}
        if current_topic_set == set(desired_topics):
            conn.commit()
            conn.close()
            return

        conn.execute("DELETE FROM leitner_topics WHERE child_id=?", (child_id,))
        for subject in subjects:
            for topic in get_default_topics(subject["name"], 5):
                conn.execute(
                    "INSERT INTO leitner_topics (child_id, subject, topic) VALUES (?,?,?)",
                    (child_id, subject["name"], topic),
                )
    conn.commit()
    conn.close()
